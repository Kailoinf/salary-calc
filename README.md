...
手动算薪工具：输入工时与奖励，即时计算到手工资，支持导出图片。含社保与个税。
...
| C 班 | 休息日（默认周三） | 手动填 C班小时 × 2 倍时薪；开 上13休1 后隔一个上一个自动算 |
| F 班 | 法定节假日 | 11h × 3 倍时薪 |

时薪 = 底薪 ÷ 21.75 ÷ 8。夜班补贴 20 元/天。

自动填充规则：当月第一个休息日之前的出勤日沿用上月班型，之后用本月班型；节假日与休息日冲突时后移。发放日规则：15 号发薪，逢周六提前到 14 号、逢周日推迟到 16 号；发薪日当天及之前显示上月（发的是上月工资），发薪日次日起显示当月。
