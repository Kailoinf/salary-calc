...
| C 班 | 固定休息日 | 手动填 C班小时 × 2 倍时薪；开 14休1 时隔一个上一个自动算 |
| F 班 | 法定节假日 | 11h × 3 倍时薪 |
...
**14休1**：开启后 C 班隔一个上一个（第一个 C 班上班），自动工时 = ceil(当月 C班数/2) × 11h，与手动 C班小时相加；关闭开关自动工时归 0，手动 C班加班保留。

## 工资公式

```
固定薪资 = 底薪 + 岗位工资 + 全勤奖 + 绩效工资 + 奖励/惩罚
税前 = 固定薪资 + A班加班(×1.5) + B班(×2) + C班(×2) + F班(×3) + 夜班补贴
社保 = 442.80 元（固定，可关闭）
个税 = (税前 - 起征点 - 社保) × 税率（可关闭）
到手 = 税前 - 社保 - 个税
```
