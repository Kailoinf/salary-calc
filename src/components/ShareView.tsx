...
/** 根据分享数据算薪资明细行 + 到手工资。ShareView 表格页与 ?p= 图片页共用。 */
export function buildShare(data: ShareData): { rows: Row[]; netPay: number } {
  const { settings } = data;
  ...
  const rows: Row[] = [
    ...
    { label: `A班加班(${ot}h×1.5)`, amount: otPay, kind: "income" },
    { label: `B班(${bh}h×2)`, amount: bPay, kind: "income" },
    { label: `C班(${ch}h×2)`, amount: cPay, kind: "income" },
    { label: `F班(${fh}h×3)`, amount: fPay, kind: "income" },
    { label: `夜班补贴(${nd}天)`, amount: nightPay, kind: "income" },
    { label: "税前总工资", amount: grossPay, kind: "total" },
    { label: "社保扣除", amount: social, kind: "deduction" },
    { label: "个税", amount: tax, kind: "deduction" },
  ];
  return { rows, netPay };
}
...
export function ShareView({ data }: { data: ShareData }) {
  const { rows, netPay } = useMemo(() => buildShare(data), [data]);
